function showNotification(message, redirectUrl = null) {
  ...
  // Wait for user to touch or click anywhere
  document.addEventListener("click", hideNotification);
  document.addEventListener("touchstart", hideNotification);
}
...
if (response.ok) {  
  form.reset();
  // Show notification and redirect after user interaction
  showNotification("⛔ Invalid details", `index.html?id=${userId}`);
}
 else {  
